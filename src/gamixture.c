...
int gaInitializePopulation(Population* pop,int size, int dim, int numberOfLabels,int numberOfPveLabels,
                           LabelType* labelTypes, float* lowLimit, float* upLimit, RFRandom & random_number_generator, bool equalVar)
{
  int i,j;
  int mixtureSize;
  int bgVar;
  ...
  for(i = 0;i < pop->size;i++) {
    ...
    for(j = bgVar; j < mixtureSize;j++) {
      tmpRand = random_number_generator.GetFloatNormPositive();
 ...
 void gaTournamentSelection(Population* selPop,Population* pop,int elitism, RFRandom & random_number_generator)
{
  ...
  for(i = elitism;i < pop->size;i++) {
    r1 = random_number_generator.Get() % pop->size;  //rand() % pop->size;
    r2 = random_number_generator.Get() % pop->size;  //rand() % pop->size;
    r = MIN(r1,r2);
    for(j = 0;j < mixtureSize;j++) {
...
void gaBLX(Population* pop,Population* selPop,float xoverRate, int elitism, float alpha, float* lowLimit, float* upLimit, RFRandom & random_number_generator, bool equalVar)
{
  int i,j;
  int leaveAlone;
  int mixtureSize;
  ...
  for(i = elitism + leaveAlone;i < pop->size;i++) {
    par1 = random_number_generator.Get() % pop->size;  //rand() % pop->size;
    par2 = random_number_generator.Get() % pop->size;  //rand() % pop->size;
    for(j = bgVar;j < mixtureSize;j++) {
      b = random_number_generator.GetFloatNormPositive(); //(float) rand()/RAND_MAX;
      b = (1 + 2*alpha)*b - alpha;
